...
def GetPotentialParallel(pos, tree, softening=None, G=1.0, theta=0.7):
    if softening is None:
        softening = zeros(pos.shape[0])
    result = empty(pos.shape[0])
    for i in prange(pos.shape[0]):
        result[i] = G * PotentialWalk(pos[i], tree, 0.0, softening=softening[i], theta=theta)
    return result


@njit(fastmath=True)
def GetPotential(pos, tree, softening=None, G=1.0, theta=0.7):
    if softening is None:
        softening = zeros(pos.shape[0])
    result = empty(pos.shape[0])
