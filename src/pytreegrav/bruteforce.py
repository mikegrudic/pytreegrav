...
def PotentialTarget_bruteforce(x_target, softening_target, x_source, m_source, softening_source,G=1.):
    ...
    for i in prange(x_target.shape[0]):
        for j in range(x_source.shape[0]):
            for k in range(3): dx[k] = x_target[i,k]-x_source[j,k]
...
@njit(fastmath=True)
def Potential_bruteforce(x,m,softening,G=1.):
    ...
    for i in range(x.shape[0]):
        for j in range(i+1,x.shape[0]):
            for k in range(3): dx[k] = x[i,k]-x[j,k]
            r = sqrt(dx[0]*dx[0] + dx[1]*dx[1] + dx[2]*dx[2])
...
def Potential_bruteforce_parallel(x,m,softening,G=1.):
    """Returns the exact mutually-interacting gravitational potential for a set of particles with positions x and masses m, evaluated by brute force.
    ...
    for i in prange(x.shape[0]):
        ...
        for j in range(x.shape[0]):
            if i==j: continue # neglect self-potential
            for k in range(3): dx[k] = x[i,k]-x[j,k]
            r = sqrt(dx[0]*dx[0] + dx[1]*dx[1] + dx[2]*dx[2])
            h = max(softening[i],softening[j])
            if r < h:
...
@njit(fastmath=True)
def Accel_bruteforce(x,m,softening,G=1.):
    ...
    for i in range(x.shape[0]):
        for j in range(i+1,x.shape[0]):
            h = max(softening[i],softening[j]) # if there is overlap, we symmetrize the softenings to maintain momentum conservation
            r2 = 0
            for k in range(3):
                dx[k] = x[i,k]-x[j,k]
                r2 += dx[k]*dx[k]
...
@njit(fastmath=True,parallel=True)
def Accel_bruteforce_parallel(x,m,softening,G=1.):
    """Returns the exact mutually-interacting gravitational accelerations of a set of particles.
...
def AccelTarget_bruteforce(x_target, softening_target, x_source, m_source, softening_source, G=1.):
    ...
    for i in prange(x_target.shape[0]):
        dx = zeros(3)
        for j in range(x_source.shape[0]):
            h = max(softening_target[i],softening_source[j]) # if there is overlap, we symmetrize the softenings to maintain momentum conservation
            r2 = 0            
            for k in range(3):
