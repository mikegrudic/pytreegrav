...
for f in argv[1:]:
    with h5py.File(f,"r") as F:
        ...
        rbins, mbin = DensityCorrFunc(x,m,rbins,theta=0.7,parallel=True,boxsize=boxsize,weighted_binning=False)
        rhocorr = mbin / np.diff(4*np.pi*rbins**3 / 3) # convert from mass in bins to average density profile around a point
        rbins, Sv = VelocityStructFunc(x,vol,v,rbins,theta=0.7,parallel=True,boxsize=boxsize,weighted_binning=False)
        rbins, SB = VelocityStructFunc(x,vol,B,rbins,theta=0.7,parallel=True,boxsize=boxsize,weighted_binning=False)
        rbins, vcorr = VelocityCorrFunc(x,vol,v,rbins,theta=0.7,parallel=True,boxsize=boxsize,weighted_binning=False)
        rbins, Bcorr = VelocityCorrFunc(x,vol,B,rbins,theta=0.7,parallel=True,boxsize=boxsize,weighted_binning=False)
